...
A1: This means that the reads are long enough to pick up the primers at the ends. We will use a function to trim them out, specifically Mergepairs functon, using trimOverhang=T
...
A2: We want to trim where the quality drops off, so around the last few nucleotides (bp 240)
...
A3: The quality of the reverse reads is usually poorer than the forward reads. This quality is acceptable as long as we trim more off toward the end. DADA2 incorporates quality information, so it ends up being okay.

Q4: Where will we trim our reverse reads and why?
A4: We will trim it at ~150 bp because that is where the quality drops off.
...
A5: N is any ambigous position where the sequencer does not know which nucleotide to put it in. The maxN=0 part of the code treats this issue and removes N's.
...
A7: The estimated error rates are a good fit for the actual error rates. There is also a drop in error as we increase the quality score.
...
A9: The nmatch column tells us the number of bases that match in the overlap region.
