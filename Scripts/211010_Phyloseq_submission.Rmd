...
A1: By using the sub_ps function, we are subsetting from the taxa table and remove any organisms that belong to the mitochondria family as well as anything from the cholorplast order. 
...
A2: Sequencing depth means the amount of samples that was read an x amount of times. For example the first bar shows 2 samples that have been read between 5500-6500 times.
...
A3: No subsampling did not have a large effect on beta diversity because the mean values before and after subsampling are very close in value, meaning subsampling has a small effect.
...
A4: Data from the NOT subsetted data set was plotted along with subsampled data set. Since the two plots look similar, we will move forward with the standardized where we normalized the read counts. 
...
A5: At time 0, the communities are similar to each other because they are close in distance. However, over time, they become more dissimilar and spread further apart. This causes beta diversity to increase. The ash leachate and control samples are clustered closely together in terms of time scale: time 0 both are similar and t4 communities are also similar for both treatment and control.
...
A6: The plots show that the control sample has a lot less variability compared to the ash leachate treatment. Furthermore, the control tends to have higher average levels of diversity than ash leachate. 3 out of 4 plots have statistically significant data. Over time, richness decreased over time but Shannon index did not vary much over time.
...
A7: Alteromonodales and Oceanspirllales both increase in abundance in the ash leachate treatment but not in the control. On the other hand, Cellvibrionales decreases in the ash leachate treatment but remains high in the control.
