...
```{r message = F}
count.tab <- read_rds("~/Desktop/144l_students_2021/Input_Data/week 9/seqtab-nochimtaxa.rds") #table of counts for each sequence in each sample, update the file path for the .rds file you generated with dada2
tax.tab <- read_rds("~/Desktop/144l_students_2021/Input_Data/week 9/taxa.rds") #table that matches ASV to sequence, update file path to your own file
...
metadata_OG <- read_excel("~/Desktop/144l_students_2021/Input_Data/week 9/144L_2021_metadata.xlsx", sheet = "Metadata")
glimpse(metadata_OG)
metadata <- metadata_OG %>%
  select(-Datetime)
glimpse(metadata)
#you can get this file from your own week6 data
metadata_doc <- read_rds("~/Desktop/144l_students_2021/Output_Data/week6/144L_Exp_Processed_BactAbund.rds")
glimpse(metadata_doc)
metadata_mock <- read_csv("~/Desktop/144l_students_2021/Input_Data/week 9/144L_2018_metadata.csv")
mock_names <- colnames(metadata_mock)
doc_names <- colnames(metadata_doc)
common_names <- intersect(mock_names, doc_names)
#glimpse(metadata_mock)
join1 <- rbind(metadata_mock[common_names], metadata_doc[common_names])
glimpse(join1)
join2 <- join1 %>%
  select(-Datetime)
glimpse(join2)
...
```{r}
saveRDS(sweet.tab, "~/Desktop/144l_students_2021/Output_Data/week 9/Custom_ASV_Table.rds")
saveRDS(sub_ps, "~/Desktop/144l_students_2021/Output_Data/week 9/phyloseq_obj.rds")
saveRDS(ps_min, "~/Desktop/144l_students_2021/Output_Data/week 9/subsampled_phyloseq_obj.rds")
saveRDS(alphadiv, "~/Desktop/144l_students_2021/Output_Data/week 9/alphadiv.rds")
...
```{r}
HowManydivision <- length(unique(gen.top[[53]]))
